...
- [4. Run the coordinator](#4-run-the-coordinator)
- [5. Options](#5-options)
...
> 🚨 **WARNING:** Ensure you have enough balance to register and run. For testnet, use the faucet (see your network’s docs).

**Step 3: Register on the network**

Acquire a slot by registering (use your network’s registration command), for example:
...
- Create a postgres database (use the name required by the project configuration)
...
**Step 4: Start PM2 with the coordinator config**
...
**Step 5: Check the coordinator is running (optional)**
...
The network identifier (e.g. 247).
...
Data directory for this node. Default: `validator`
...
The number of workers to query in a single step.
...
The maximum balance allowed for processing coordinator responses (e.g. 1000).
...
| Command                          | Description                     |
| -------------------------------- | ------------------------------- |
| `pm2 stop validator`             | Stops the coordinator.            |
| `pm2 logs validator --lines 100` | View the coordinator logs. |
...
## Run the coordinator with Docker

You can run the coordinator with Docker and docker compose.
...
- List databases: `\l`. Connect to the project database: `\c <db_name>`
