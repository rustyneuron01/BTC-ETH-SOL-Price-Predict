...
Visit the project dashboard for worker performance and stats.
...
#### 3. What does it mean when I check the validation endpoint and see: `{"validated":false,"reason":"Number of time points is incorrect: expected 289, got 288","response_time":"4.07"}`?
...
`synth/validator/response_validation_v2.py`
...
#### 6. The system predicts multiple assets. How does each asset prediction contribute to the smoothed score?

Each asset—BTC, ETH, XAU and SOL—contributes equally to the smoothed score. Workers must perform consistently well across all four. New assets added to the system will have the same weighting unless noted otherwise.
...
The project may publish performance reviews and strategy notes. Check the project resources for updates.
...
#### 13. Is the system running on a testnet too?
...
`synth/miner/simulations.py` in the repository
